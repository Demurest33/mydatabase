<x-admin-layout>
    <x-slot:title>Edit Album - Backoffice</x-slot>

    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-white">Edit Album</h1>
            <p class="text-gray-400 mt-2">{{ $album['name'] }}</p>
        </div>
            ...
            <div class="lg:col-span-1 space-y-5">
                <div class="bg-[#151921] border border-gray-800 rounded-2xl p-6 shadow-xl space-y-5">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" required value="{{ old('name', $album['name']) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full bg-black/40 border border-gray-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition-colors text-sm">{{ old('description', $album['description'] ?? '') }}</textarea>
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Cover Image URL</label>
                        <input type="url" name="coverImage" value="{{ old('coverImage', $album['coverImage'] ?? '') }}"
                               class="w-full bg-black/40 border border-gray-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition-colors text-sm"
                               placeholder="https://...">
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <input type="checkbox" name="isActive" id="isActive" value="1"
                               {{ old('isActive', $album['isActive'] ?? true) ? 'checked' : '' }}
                               class="w-4 h-4 rounded accent-violet-500 cursor-pointer">
                        <label for="isActive" class="text-sm text-gray-400 cursor-pointer">Álbum activo (visible públicamente)</label>
                    </div>
                    ...
                    <div class="pt-4 border-t border-gray-800 space-y-3">
                        ...
                        <a href="{{ route('albums.show', $album['id']) }}" target="_blank"
                           class="block w-full text-center bg-black/40 hover:bg-violet-600/20 border border-gray-700 hover:border-violet-500/50 text-gray-400 hover:text-violet-300 font-bold py-2.5 px-6 rounded-xl transition-colors text-sm">
                            Ver álbum →
                        </a>
                    </div>
                </div>
                            ...
                            <div class="franchise-group" data-franchise="{{ strtolower($franchise) }}">
                                ...
                                <div class="space-y-1">
                                    @foreach($items as $item)
                                        @php $checked = in_array($item['id'], old('media_ids', $selectedMediaIds)); @endphp
                                        <label class="media-item flex items-center gap-3 p-2.5 rounded-xl hover:bg-black/30 cursor-pointer transition-colors group {{ $checked ? 'bg-violet-500/5 border border-violet-500/20' : '' }}"
    ...
    (function () {
        ...
        checkboxes.forEach(cb => {
            cb.addEventListener('change', function () {
                this.closest('label').classList.toggle('bg-violet-500/5', this.checked);
                this.closest('label').classList.toggle('border', this.checked);
                this.closest('label').classList.toggle('border-violet-500/20', this.checked);
                updateCount();
            });
        });
        updateCount();
        ...
        document.getElementById('btn-select-all').addEventListener('click', () => {
            document.querySelectorAll('.media-item:not([hidden]) input').forEach(cb => {
                cb.checked = true;
                cb.closest('label').classList.add('bg-violet-500/5', 'border', 'border-violet-500/20');
            });
            updateCount();
        });
        ...
        document.getElementById('btn-clear-all').addEventListener('click', () => {
            checkboxes.forEach(cb => {
                cb.checked = false;
                cb.closest('label').classList.remove('bg-violet-500/5', 'border', 'border-violet-500/20');
            });
            updateCount();
        });
